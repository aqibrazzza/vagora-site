...
- **Preloader: the mark first, and as tall as the phrase.** The mark is now sized to the cap height of "Beyond the Mirror" — measured by `fitLine()` from the fitted type, so it scales with the phrase at every width (≈126px at 1440, ≈227px at 2560; it was 42–50px wide). On phones the same rule left it too small, so it is twice the cap height at ≤480px (≈68px at 390), easing to 1× by 1100px. The sequence is authored in order: the mark alone resolves in place — slowly, as one breath (opacity 1s `power1.inOut`, scale 1.06 → 1 over 1.2s `power2.out`; an expo arrival read as static, a longer one as a dead tail) — then the phrase rises from 1.05s, then the lift. The card is ≈0.85s longer as a result. §01, §10 vocabulary, §11 and `assets.logo.inProduction.preloaderMark` follow.
